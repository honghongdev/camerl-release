...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        if self.use_rnn == False:
            print("Skipping RNN — using CNN+state→MLP directly")
        else:
            print("Using RNN for training")
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        # action_dist = <class 'stable_baselines3.common.distributions.DiagGaussianDistribution'>
        # print("action_dist =", type(self.action_dist))
        # import sys
        # sys.exit()
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        """
        # policy_net:  [feature_dim] → Linear(feature_dim, 256) → Act → Linear(256, 256) → Act
        # value_net:   [feature_dim] → Linear(feature_dim, 512) → Act → Linear(512, 512) → Act
        
        if self.use_rnn:
            head_input_dim = self.lstm_output_dim + 7
        else:
            head_input_dim = self.features_dim + 7
    ...
    def forward_cnn(self, obs):
        ...
        cat_vf = []
        for key, _obs in obs.items():
            # print(f"[forward_cnn] key={key}, shape={_obs.shape}")
            if key == 'image':
                features = self.extract_features(_obs)
                pi_features = vf_features = features
            else:
                # print("raw _obs.shape:", _obs.shape)
                state_shape = _obs.shape
                # (B, 1, S) --> (B, S)， S = 7
                _obs = _obs.reshape([state_shape[0], state_shape[2]]).float()
    ...
    def predict_values_cnn(self, obs):
        _, latent_vf = self.forward_cnn(obs)
        latent_vf_ = self.mlp_extractor.forward_critic(latent_vf)
        return self.value_net(latent_vf_)
    ...
    def predict(
        ...
        """
        if self.use_rnn == False:
            return self.predict_cnn(observation, deterministic=deterministic)
    ...
    def predict_cnn(
        ...
        """
        CNN-only inference: bypasses RNN, does not update or depend on LSTM hidden state.
        ...
        with th.no_grad():
            latent_pi, latent_vf = self.forward_cnn(observation)
            actions, _, _ = self.forward(latent_pi, latent_vf, deterministic=deterministic)
